    ...
    <div className="grid place-items-center">
      <div className="clinic-card w-full max-w-xl p-8 text-center">
        ...
        <div className="mt-5 flex justify-center gap-2">
          <button onClick={() => navigate("/calendar?new=1")} className="rounded-xl border border-clinic-border px-4 py-2">{t("confirmation.addAnotherAppointment")}</button>
          <button onClick={() => navigate("/calendar")} className="rounded-xl bg-clinic-primary px-4 py-2 text-white">{t("confirmation.viewCalendar")}</button>
        </div>
      </div>
    </div>
  );
